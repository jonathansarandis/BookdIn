...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: job, error: jobError } = await admin
    .from('jobs')
    .select('id, business_id, scheduled_at, customer:customers(stripe_customer_id), business:businesses(timezone)')
    .eq('id', params.id)
  ...
  if (!job.customer?.stripe_customer_id) {
  ...
  const { error: updateError } = await admin
    ...
    .update({
      ...
      stripe_customer_id: job.customer.stripe_customer_id,
